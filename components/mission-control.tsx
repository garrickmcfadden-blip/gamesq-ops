...
export function MissionControl() {
  ...
  function submitMatter() {
    ...
    setMatterForm({ title: '', client: '', stage: 'Intake', priority: 'medium', status: '', owner: 'Garrick', nextAction: '', blocker: '', projectedValue: '', incidentDate: '', statute: '', claimNumber: '', adjusterName: '', adjusterPhone: '' });
        ...
        {showNewMatter ? (
          <Card title="New Matter Intake" subtitle="Create a real matter record in Mission Control" className="border-gam-peach/20">
            <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
              ...
              <input value={matterForm.claimNumber} onChange={(e) => setMatterForm({ ...matterForm, claimNumber: e.target.value })} placeholder="Claim number" className="rounded-xl border border-gam-blue/10 bg-white/70 px-3 py-2 text-sm text-gam-blue outline-none" />
              <input value={matterForm.adjusterName} onChange={(e) => setMatterForm({ ...matterForm, adjusterName: e.target.value })} placeholder="Adjuster name" className="rounded-xl border border-gam-blue/10 bg-white/70 px-3 py-2 text-sm text-gam-blue outline-none" />
              <input value={matterForm.adjusterPhone} onChange={(e) => setMatterForm({ ...matterForm, adjusterPhone: e.target.value })} placeholder="Adjuster phone" className="rounded-xl border border-gam-blue/10 bg-white/70 px-3 py-2 text-sm text-gam-blue outline-none" />
            ...
            <Card title={panels.detail.title} subtitle={panels.detail.subtitle} className="border-gam-peach/20">
              {selectedMatter ? <div className="space-y-5">
                <div className="rounded-2xl border border-gam-peach/20 bg-gam-orange/5 p-4">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                    <div className="rounded-xl border border-gam-blue/10 bg-white/75 p-3"><div className="text-[11px] uppercase tracking-[0.2em] text-gam-blue/45">Claim Number</div><div className="mt-2 text-sm font-semibold text-gam-blue">{selectedMatter.claimNumber || '—'}</div></div>
                    <div className="rounded-xl border border-gam-blue/10 bg-white/75 p-3"><div className="text-[11px] uppercase tracking-[0.2em] text-gam-blue/45">Crash Date</div><div className="mt-2 text-sm font-semibold text-gam-blue">{formatDateDisplay(selectedMatter.incidentDate)}</div></div>
                    <div className="rounded-xl border border-gam-blue/10 bg-white/75 p-3"><div className="text-[11px] uppercase tracking-[0.2em] text-gam-blue/45">Adjuster</div><div className="mt-2 text-sm font-semibold text-gam-blue">{selectedMatter.adjusterName || '—'}</div></div>
                    <div className="rounded-xl border border-gam-blue/10 bg-white/75 p-3"><div className="text-[11px] uppercase tracking-[0.2em] text-gam-blue/45">Adjuster Phone</div><div className="mt-2 text-sm font-semibold text-gam-blue">{selectedMatter.adjusterPhone || '—'}</div></div>
                  </div>
                  <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
                    ...
                    <label className="rounded-xl bg-white/70 p-3"><div className="text-xs uppercase tracking-[0.2em] text-gam-blue/45">Claim Number</div><input value={selectedMatter.claimNumber ?? ''} onChange={(e) => updateMatter(selectedMatter.id, { claimNumber: e.target.value })} className="mt-2 w-full border-0 bg-transparent text-sm text-gam-blue outline-none" placeholder="Carrier claim number" /></label>
                    ...
                    <label className="rounded-xl bg-white/70 p-3"><div className="text-xs uppercase tracking-[0.2em] text-gam-blue/45">Adjuster Name</div><input value={selectedMatter.adjusterName ?? ''} onChange={(e) => updateMatter(selectedMatter.id, { adjusterName: e.target.value })} className="mt-2 w-full border-0 bg-transparent text-sm text-gam-blue outline-none" placeholder="Assigned adjuster" /></label>
                    <label className="rounded-xl bg-white/70 p-3"><div className="text-xs uppercase tracking-[0.2em] text-gam-blue/45">Adjuster Phone</div><input value={selectedMatter.adjusterPhone ?? ''} onChange={(e) => updateMatter(selectedMatter.id, { adjusterPhone: e.target.value })} className="mt-2 w-full border-0 bg-transparent text-sm text-gam-blue outline-none" placeholder="Direct line" /></label>
